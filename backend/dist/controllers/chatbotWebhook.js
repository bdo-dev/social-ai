...
const fileManager = new GoogleAIFileManager("AIzaSyCpEpuWarkbYHyt7U8XG_9vhi7Wsy5sbJU");
...
async function likeComment(commentId) {
  ...
  try {
    ...
    console.error("Error liking comment:", error.response ? error.response.data : error.message);
  }
}
        ...
        for (const webhookEvent of entry.messaging) {
          ...
          if (webhookEvent.message) {
            if (webhookEvent.message.text) {
              ...
              promises.push(callGeminiAPI(session).then(response => {
                updateSession(senderId, {
                  role: "model",
                  parts: [{
                    text: response
              ...
              }).catch(error => console.error("Error handling text message:", error)));
            }
            if (webhookEvent.message.attachments) {
              for (const attachment of webhookEvent.message.attachments) {
                if (attachment.type === "audio") {
                  ...
                  promises.push(handleAudio(audioUrl).then(response => sendMessengerMessage(senderId, response)).catch(error => console.error("Error handling audio message:", error)));
                } else {
                  promises.push(sendMessengerMessage(senderId, "I currently support text and audio messages only."));
                }
              }
            }
          }
      ...
      if (entry.changes) {
        for (const change of entry.changes) {
          if (change.field === "feed") {
            ...
            if (pageId !== usrId) {
              if (change.value.message === undefined && change.value.item !== "reaction") continue;
              // console.log(change.value?.reaction_type);
              console.log(change.value);
              if (change.value?.reaction_type !== undefined) {
                if (change.value.verb === "edit") continue;
                addOrRemoveLike(change.value?.parent_id, change.value.verb === "remove");
              } else {
                hadnleSentiment(change);
                const replyMessage = `Thank you for your comment: "${commentMessage}"!`;
                promises.push(replyToComment(commentId, replyMessage).then(() => likeComment(commentId)).catch(error => console.error("Error handling comment:", error)));
              }
            }
          }
        }
...
async function callGeminiAPI(session) {
  try {
    const chatSession = model.startChat({
      generationConfig,
      history: session // Use the session history
    });
    const result = await chatSession.sendMessage(session[session.length - 1].parts[0].text);
    return result.response.text();
  } catch (error) {
...
const hadnleSentiment = async comment => {
  try {
    ...
    const {
      ...
      isHidden: false,
      comment_id_facebook: comment.value.parent_id
    });
    if (error) log(error);
  } catch (error) {
...
const fetchPostId = async idFacebook => {
  const {
    data,
    error
  } = await supabase.from("posts").select("id").eq("id_post_facebook", idFacebook);
...
const addOrRemoveLike = async (comment_id, isRemove) => {
  ...
  const {
    data,
    error
  } = await supabase.from("comments").select("id").eq("comment_id_facebook", comment_id);
  if (data === null) return;
  if (error) {
    console.log(error);
  }
  const {
    data: comment,
    error: errorComment
  } = await supabase.rpc("comment_count", {
    comment_id,
    should_decrement: isRemove
  });
  if (errorComment) console.error(errorComment);else console.log(comment, isRemove);
};