...
const accessTokenIN =
  "EAAcUGHPIN28BO538b1RfrMUydPyzL0voWhtFafrZCHHEQmxCHEu2z92l6cP31mrF3tMMfCoV751N6NNGOvdL5ZAYBLuZBS24GULLDiRzmw8LZC3ltTe4ygCwQbwmt6UTyNkzsB3E0eRUgNWbOQFoJcImrQzxrXYDlMnSDWZBRivpqmYNexXAHzPbnIpquZAu9nNZB3pjhmq";
const instagramUserId = "17841440091785604";
export const publishPostToInstagram = async (post) => {
  ...
  try {
    ...
    for (const mediaUrl of media_urls) {
      ...
      const createMediaUrl = `https://graph.facebook.com/v18.0/${instagramUserId}/media`;
    ...
    const publishMediaUrl = `https://graph.facebook.com/v18.0/${instagramUserId}/media_publish`;
    const publishResponse = await axios.post(publishMediaUrl, null, {
      params: {
        creation_id: mediaIds[0],
        // Assuming single media for simplicity
        access_token: accessTokenIN,
      },
    });
    console.log(3);
    ...
    let errorMessage =
      "An unexpected error occurred while publishing the post to Instagram.";
...
export const publishPost = async (post) => {
  ...
  try {
    ...
    const { data, error } = await supabase
      .from("posts")
      .update({ id_post_facebook: response.data.id })
      .eq("id", post.id);
...
export const updatePostWithMedia = async (postId, content, newMediaUrls) => {
  ...
  try {
    ...
    const updateResponse = await axios.post(postUrl, null, { params });
...
export const fetchPostComments = async (postId) => {
  ...
  try {
    const comments = await axios.get(url, { params });
...
export const getFacebookPostEngagement = async (postId) => {
  ...
  try {
    const engagement = await axios.get(url, { params });
...
export const getFacebookPostReach = async (postId) => {
  ...
  try {
    const reach = await axios.get(url, { params });
