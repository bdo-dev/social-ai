...
export const createPost = async (req, res) => {
  try {
    ...
    const { content, images, tags, platform, scheduledFor } = req.body;
...
export const updatePost = async (req, res) => {
  ...
  try {
    ...
    const isPublish = newScheduledFor
      ? new Date(newScheduledFor) <= now
      : false;
    ...
    if (postId) cancelScheduledPost(id);
    if (postId)
      if (scheduledFor) {
    ...
    if (postId) updatePostWithMedia(postId, content, images);
...
export const getPosts = async (req, res) => {
  const { page = 0, loadMore = false } = req.query;
  ...
  try {
    ...
    const { full_name, username, avatar_url } = user.user_metadata;
    ...
    res
      .status(200)
      .json({ posts: formattedPosts, hasMore: postsData.length === 10 });
  } catch (error) {
